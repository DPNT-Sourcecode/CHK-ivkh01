...
class Checkout
  ...
  def not_proper(skus)
    ...
    letters.each do |letter|
      bool = true if !(["A","B","C","D", "E", "F","G","H"].include?(letter))
    end
    bool
  end

  def hash_making_of_string(skus)
    hash = {"A"=> 0,"B"=> 0, "C"=> 0, "D"=> 0, "E"=>0 , "F" => 0, "G"=> 0 , "H" =>0, "I" => 35,
    "J" => 60  }
  ...
  def price_counting_of_hash(hash)
    ...
    item_F_price(hash)+ item_B_price(hash) + normal_price(hash, "G", 20)+item_H_price(hash)+ normal_price(hash, "I",35) + normal_price(hash, "J", 60)
